...
export async function GET(req: NextRequest) {
  ...
  if (type === "expense") {
    ...
    let sql = `
      SELECT id, expense_date as date, category as ref, title as party,
             amount as debit, 0 as credit, paid_from as source, notes, 0 as manual
      ...
      SELECT s.id, s.sale_date as date, 'Sale Bill Expense' as ref,
             ...
             COALESCE(s.invoice_no, '#' || s.id) as notes, 0 as manual
      FROM sales s
      ...
      SELECT p.id, p.purchase_date as date, 'Purchase Expense' as ref,
             ...
             COALESCE(p.invoice_no, '#' || p.id) as notes, 0 as manual
      FROM purchases p
  ...
  if (type === "salesman") {
    ...
    if (subType === "salesman-to-customer") {
      sql = `SELECT s.id, s.sale_date as date, s.invoice_no as ref,
             ...
             'Salesman -> Customer' as notes, 0 as manual
             FROM sales s
      ...
      sql = `SELECT s.id, s.sale_date as date, s.invoice_no as ref,
             ...
             'Customer -> Salesman' as notes, 0 as manual
             FROM sales s
             JOIN customers c ON c.id = s.customer_id
             LEFT JOIN salesmen sm ON sm.id = s.salesman_id
             WHERE (s.deleted IS NULL OR s.deleted = 0) AND s.salesman_id IS NOT NULL`;
      ...
      sql = `SELECT s.id, s.sale_date as date, s.invoice_no as ref,
             ...
             ' | Discount: ' || printf('%.0f', COALESCE(s.total_discount, 0)) as notes, 0 as manual
             FROM sales s
             LEFT JOIN salesmen sm ON sm.id = s.salesman_id
             JOIN customers c ON c.id = s.customer_id
  ...
  if (type === "customer") {
    ...
    let sql = `SELECT s.id, s.sale_date as date, s.invoice_no as ref,
               ...
               ' | Empty: ' || printf('%.0f', COALESCE(s.empty_qty, 0)) as notes, 0 as manual
               FROM sales s
               JOIN customers c ON c.id = s.customer_id
  ...
  const sql = `
    SELECT p.id as id, p.purchase_date as date,
           ...
           'Stock into warehouse/floor' as notes, 0 as manual
    FROM purchases p
    ...
    SELECT s.id, s.sale_date as date,
           ...
           'Sale OUT' as source,
           COALESCE(sm.name, '-') || ' | Empty return: ' || printf('%.0f', COALESCE(s.empty_qty, 0)) as notes, 0 as manual
    FROM sales s
    JOIN customers c ON c.id = s.customer_id
    LEFT JOIN salesmen sm ON sm.id = s.salesman_id
    ...
    SELECT st.id, st.transfer_date as date,
           ...
           'Transfer' as source, st.notes, 0 as manual
    ...
    SELECT sa.id, sa.adjust_date as date,
           ...
           'Adjustment' as source, sa.notes, 0 as manual
